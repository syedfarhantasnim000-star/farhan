...
function Carousel() {
  ...
  // Safety fallback only — real advancement is via onEnded
  useEffect(() => {
    const fallback = setTimeout(() => go(1), 9000);
...
function HeroSection() {
  ...
  /* Mobile: simple boolean — CSS handles the transition, no JS scroll listener */
  ...
  useEffect(() => {
    const t = setInterval(() => {
      glowIdx.current = (glowIdx.current + 1) % GLOW_COLORS.length;
      setGlowColor(GLOW_COLORS[glowIdx.current]);
    }, 6000);
    ...
    return (
      <section style={{ position: "relative", height: "200vh" }}>
        <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden", background: "#000" }}>
          <div className="hero-vid"><Carousel /></div>
          {Title}
          {ScrollCue}
        </div>
      </section>
